...
>>>>>>> bd5aeb2a21fbc437f1b322610022f7cb1ba6a9db
  ...
  Wish List
	...
	o as writing time is measure for all files at once, the display
	  cannot be instrct_grp_ana_vals.Dependently written to a file via signals.par, while
...
#include "optimizer_cmaes_interface.h" /* <time.h> via cmaes.h */
...
/* TODO: make signals.par another argument or, even better, part of cmaes_t */
{
    ...
    while(!(stop = cmaes_TestForTermination(evo)) &&
        ...
        /* read strct_grp_ana_vals.Control signals for output and termination */
        if(signalsFilename)
...
cmaes_Get(cmaes_t* t, char const* s)
{
    ...
    if(strncmp(s, "axisratio", 5) ==
       0) {  /* between lengths of longest and shortest principal axis of the distribution ellipsoid */
...
cmaes_TestForTermination(cmaes_t* t)
{
    ...
    /* Principal axis i has no strct_grp_ana_vals.Effect on xmean, ie.
    ...
    if(!flgdiag) {
        for(iAchse = 0; iAchse < N; ++iAchse) {
            ...
            if(iKoo == N) {
                /* t->sigma *= exp(0.2+t->sp.cs/t->sp.damps); */
                cp += sprintf(cp,
                              "Nostrct_grp_ana_vals.EffectAxis: standard deviation 0.1*%7.2e in principal axis %d without strct_grp_ana_vals.Effect\n",
                              fac / 0.1, iAchse);
                break;
            } /* if (iKoo == N) */
        } /* for iAchse             */
    ...
    for(iKoo = 0; iKoo < N; ++iKoo) {
        if(t->rgxmean[iKoo] == t->rgxmean[iKoo] +
           0.2 * t->sigma * sqrt(t->C[iKoo][iKoo])) {
            ...
            /* flg = 1; */
            cp += sprintf(cp,
                          "Nostrct_grp_ana_vals.EffectCoordinate: standard deviation 0.2*%7.2e in coordinate %d without strct_grp_ana_vals.Effect\n",
