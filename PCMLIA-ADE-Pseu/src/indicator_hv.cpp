...
static void delete_dlnode(dlnode_t* nodep, int dim, double* bound __variant3_only)
{
    int i;
...
double generateHV(char* _testName, char* _algoName, double* _estedPF, int _nEstedPF, int _nObj, int _nDim, int* iKey,
                  int iRun)
{
    int nobj = _nObj;
    // int ndim = _nDim;
    int np = _nEstedPF;
    double* refp = (double*)calloc(nobj, sizeof(double));
    double maxVol = 1.0;

    if(_nObj > 3 && strcmp(_testName, "EdgeComputation")) {
        return -10.0;
    }
    if(!strcmp(_testName, "UF1") ||
       !strcmp(_testName, "UF2") ||
       !strcmp(_testName, "UF3") ||
       !strcmp(_testName, "UF4")) {
        for(int i = 0; i < nobj; i++) {
            refp[i] = 4.0e0;
        }
    } else if(!strcmp(_testName, "UF5")) {
        for(int i = 0; i < nobj; i++) {
            refp[i] = 5.0e0;
        }
    } else if(!strcmp(_testName, "UF6")) {
        for(int i = 0; i < nobj; i++) {
            refp[i] = 7.0e0;
        }
    } else if(!strcmp(_testName, "UF7")) {
        for(int i = 0; i < nobj; i++) {
            refp[i] = 3.0e0;
        }
    } else if(!strcmp(_testName, "UF8")) {
        for(int i = 0; i < nobj; i++) {
            refp[i] = 4.0e0;
        }
    } else if(!strcmp(_testName, "UF9")) {
        for(int i = 0; i < nobj; i++) {
            refp[i] = 3.0e0;
        }
    } else if(!strcmp(_testName, "UF10")) {
        for(int i = 0; i < nobj; i++) {
            refp[i] = 4.0e0;
        }
    } else if(!strcmp(_testName, "DTLZ1")) {
        for(int i = 0; i < nobj; i++) {
            refp[i] = 2.0e3;
        }
    } else if(!strcmp(_testName, "DTLZ2")) {
        for(int i = 0; i < nobj; i++) {
            refp[i] = 1.5e1;
        }
    } else if(!strcmp(_testName, "DTLZ3")) {
        for(int i = 0; i < nobj; i++) {
            refp[i] = 5.0e3;
        }
    } else if(!strcmp(_testName, "DTLZ4")) {
        for(int i = 0; i < nobj; i++) {
            refp[i] = 1.0e1;
        }
    } else if(!strcmp(_testName, "DTLZ5")) {
        for(int i = 0; i < nobj; i++) {
            refp[i] = 1.0e1;
        }
    } else if(!strcmp(_testName, "DTLZ6")) {
        for(int i = 0; i < nobj; i++) {
            refp[i] = 7.0e2;
        }
    } else if(!strcmp(_testName, "DTLZ7")) {
        for(int i = 0; i < nobj; i++) {
            refp[i] = 1.6e1;
        }
    } else if(!strcmp(_testName, "WFG1") ||
              !strcmp(_testName, "WFG2") ||
              !strcmp(_testName, "WFG3") ||
              !strcmp(_testName, "WFG4") ||
              !strcmp(_testName, "WFG5") ||
              !strcmp(_testName, "WFG6") ||
              !strcmp(_testName, "WFG7") ||
              !strcmp(_testName, "WFG8") ||
              !strcmp(_testName, "WFG9")) {
        for(int i = 0; i < nobj; i++) {
            refp[i] = 1e1;
        }
    } else if(!strcmp(_testName, "LSMOP1") ||
              !strcmp(_testName, "LSMOP2") ||
              !strcmp(_testName, "LSMOP3") ||
              !strcmp(_testName, "LSMOP4") ||
              !strcmp(_testName, "LSMOP5") ||
              !strcmp(_testName, "LSMOP6") ||
              !strcmp(_testName, "LSMOP7") ||
              !strcmp(_testName, "LSMOP8") ||
              !strcmp(_testName, "LSMOP9")) {
        for(int i = 0; i < nobj; i++) {
            refp[i] = 1e1;
        }
    } else if(!strcmp(_testName, "DSN")) {
        for(int i = 0; i < nobj; i++) {
            refp[i] = 1e1;
        }
    } else if(!strcmp(_testName, "RecSys_SmartCity")) {
        for(int i = 0; i < nobj; i++) {
            refp[i] = 1e0;
        }
        refp[0] = 700;
        ...
        refp[4] = 160;
        refp[5] = 7;
    } else if(!strncmp(_testName, "FRNN", 4) ||
    ...
    double hv = fpli_hv(_estedPF, nobj, np, refp);

    /*    char fileName[256];
    	if(iKey)
    	sprintf(fileName,"HV/trace/HV_%s_%s_OBJ%d_VAR%d_RUN%d",_algoName,_testName,nobj,ndim,iRun);
    	else
    	sprintf(fileName,"HV/HV_%s_%s_OBJ%d_VAR%d_RUN%d",_algoName,_testName,nobj,ndim,iRun);
